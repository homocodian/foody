...
export default function Snackabr({ variant, title, text, setOpen }: Snackabr) {
  ...
  const getColor = () => {
    if (variant === "Info") return 'bg-dark-blue';
    else if (variant === "Success") return "bg-green";
    else if (variant === "Alert") return "bg-yellow";
  ...
  return (
    <div className={`${variantColor} fixed left-2 bottom-1 shadow-lg mx-auto w-72 md:w-96 max-w-full text-sm pointer-events-auto bg-clip-padding rounded-[5px] block mb-3`} id="alert" role="alert" aria-live="assertive" aria-atomic="true" data-mdb-autohide="false">
      <div className={`${variantColor} flex justify-between items-center py-2 px-3 bg-clip-padding border-b border-white rounded-t-lg`}>
      ...
      <div className={`p-3 ${variantColor} rounded-b-lg break-words text-white`}>
        {text}
      </div>
    </div>
  )
